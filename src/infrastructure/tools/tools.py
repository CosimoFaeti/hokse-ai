...
async def get_activities(
	...
	*,
	athlete_id: Annotated[int, InjectedState("athlete_id")],
) -> str:
	...
	if mode == "list":
		...
		for a in activities:
			...
			pace_str: str = f" | {a.pace_min_per_km:.2f} min/km" if a.pace_min_per_km and a.sport_type == "Run" else ""
			hr_str: str = f" | HR {a.average_heartrate:.0f} bpm" if a.average_heartrate else ""
			elev_str: str = f" | +{a.total_elevation_gain:.0f} m" if a.total_elevation_gain > 0 else ""
			lines.append(
