...
class StravaTokenEntity(BaseModel):
	...
	athlete_id: int | None = Field(title="strava_athlete_id", description="Strava athlete id for Strava API.")

	access_token: str | None = Field(title="access_token", description="Access token for Strava API.")

	refresh_token: str | None = Field(title="refresh_token", description="Refresh token for Strava API.")

	expires_at: datetime | None = Field(
		title="expires_at", description="Expiration time for Strava API (stored as UTC datetime)."
	)

	scope: str | List[str] = Field(title="scope", description="Scopes for Strava API.")
