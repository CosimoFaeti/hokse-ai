		...
		match authentication_type:
			case AuthenticationType.TOKEN:
				...
				if not token:
					...
					raise HTTPException(
						...
						detail=f"No token provided for request to route {route.path}.",
					)
				if not validate_jwt(token):
					logger.error(msg=f"Provided token for request to route {route.path} is not valid.")
					raise HTTPException(
						status_code=status.HTTP_401_UNAUTHORIZED,
						detail=f"Provided token for request to route {route.path} is not valid.",
					)
			case AuthenticationType.API_KEY:
				api_key = request.headers.get("X-API-Key")
				if not api_key:
					logger.error(msg=f"No API KEY provided for request to route {route.path}.")
					raise HTTPException(
						status_code=status.HTTP_401_UNAUTHORIZED,
						detail=f"No API KEY provided for request to route {route.path}.",
					)
				if not validate_api_key(api_key):
					logger.error(msg=f"Provided API KEY for route {route.path} is not valid.")
					raise HTTPException(
						status_code=status.HTTP_401_UNAUTHORIZED,
						detail=f"Provided API KEY for route {route.path} is not valid.",
					)
			...
			case _:
				...
				raise HTTPException(
					...
					detail=f"Authentication type {authentication_type} for route {route.path} is not supported",
				)
